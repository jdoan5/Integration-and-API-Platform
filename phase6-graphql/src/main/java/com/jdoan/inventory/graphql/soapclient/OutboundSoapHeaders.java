...
public class OutboundSoapHeaders {
    ...
    public WebServiceMessageCallback propagate() {
        // Read the ids on the CALLING thread. The callback runs later, and
        // @BatchMapping resolves on a parallel stream - by then the thread-local
        // context belongs to a different thread, or to nobody.
